...
server <- function(input, output, session) {
  ...
  if (panel_state() == "login") {
    ...
    updateSelectInput(session, "update_with_shapefile", choices = created_variables)
  }
  })
  ...
  observeEvent(input$create_shapefile, {
    ...
    if (is.na(st_crs(new_shapefile))) {
    # Set the CRS if it's NA (not available)
    new_shapefile <- st_set_crs(new_shapefile, 2229)
    } else {
        # Transform the CRS to EPSG:2229
        new_shapefile <- st_transform(new_shapefile, 2229)
    }
    ...
    update_with_shapefile <- input$update_with_shapefile
    
    # Read the shapefiles
    existing_shapefile <- get(shapefile_to_update)
    new_shapefile_data <- processed_data[processed_data$OrgnlSN == update_with_shapefile, ]
    
    if (input$update_action == "add") {
        # Add the rows of the new shapefile to the existing shapefile
        updated_shapefile <- rbind(existing_shapefile, new_shapefile_data)
    ...
    # Set the CRS if it's NA (not available)
        ...
        existing_shapefile[existing_shapefile$OrgnlSN == update_with_shapefile, ] <- new_shapefile_data
        if (is.na(st_crs(existing_shapefile))) {
    # Set the CRS if it's NA (not available)
